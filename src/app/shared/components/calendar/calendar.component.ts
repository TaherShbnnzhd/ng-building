  ...
  implements OnInit, AfterContentInit, AfterViewInit, OnDestroy
{
  ...
  isDateEquals(value: any, dateMeta: any) {
    if (value && value instanceof JDate)
      return (
    ...
    switch (event.toState) {
      ...
      case 'visibleTouchUI':
        if (!this.inline) {
          ...
          (this.overlay as HTMLDivElement).setAttribute(
            this.attributeSelector,
            ''
          );
          this.appendOverlay();
  ...
  appendOverlay() {
    if (this.appendTo) {
      if (this.appendTo === 'body')
        document.body.appendChild(this.overlay as HTMLDivElement);
  ...
  getFirstDateOfWeek() {
    return (
      ...
      this.getTranslation(TranslationKeys.FIRST_DAY_OF_WEEK)
    );
  }
              ...
              output += formatNumber(
                ...
                Math.round(
                  (new JDate(
                    ...
                    new JDate(date.getFullYear(), 0, 0).getTime()) /
                    86400000
                ),
                3
              );
