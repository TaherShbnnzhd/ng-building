...
export class JDateValidatorService {
  ...
  public isValidJYear(jYear: number): boolean {
    return (
      jYear >= JDateValidatorService._minAcceptableYear &&
      jYear <= JDateValidatorService._maxAcceptableYear
    );
  }
  ...
  public isValidJDate(jYear: number, jMonth: number, jDay: number): boolean {
    return (
      this.isValidJYear(jYear) &&
      this.isValidJMonth(jMonth) &&
      this.isValidJDay(jYear, jMonth, jDay)
    );
  }
}
